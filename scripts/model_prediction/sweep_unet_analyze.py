"""Sweep analysis for the U-Net optimizer × LR × scheduler grid.

Walks every ``unet_sweep_<opt>_lr<lr_tag>_<sched>/`` directory under
``sweep_root`` (one per combo written by
``slurm_sweep_unet_jeanzay.sh``), reads each model's ``metrics.csv``
(the flat CSVLogger output the U-Net Lightning trainer writes), and
produces:

1. ``sweep_summary.csv`` with columns: ``experiment, optimizer,
   learning_rate, scheduler, val_loss_best, val_loss_final,
   train_loss_final, val_psnr_best, val_psnr_final, epochs_done``.
...
3. ``sweep_best_models.txt`` — the top-K runs ranked by best (lowest)
   ``val_loss`` and (separately) by best (highest) ``val_psnr``.

Edit the paths block at the top, then::

    python sweep_unet_analyze.py

Sibling script: ``sweep_unet_clean.py`` (deletes intermediate
checkpoints once you've picked the winning configuration).
"""
...
#   unet_sweep_<opt>_lr<lr_tag>_<sched>
dir_glob = "unet_sweep_*"

# Top-K to highlight in the printout / best-models file.
top_k = 5

# Primary metric ranking. Loss is always reported descending good→bad
# (lower is better); psnr the opposite. Both are emitted to the summary.
primary_metric = "val_loss_best"  # any column in the summary CSV
...
_DIR_RE = re.compile(
    r"unet_sweep_(?P<opt>[a-z]+)_lr(?P<lr_tag>[\dp+\-e]+)_(?P<sched>\w+)"
)
...
def _decode_lr_tag(lr_tag: str) -> float:
    ...
    """
    # Mantissa: digits with a ``p`` standing in for the decimal point.
    m = re.match(r"^(\d+)p(\d+)e(p?)(-?)(\d+)$", lr_tag)
    if m is None:
        return float("nan")
    int_part, frac_part, plus_token, sign, exp_digits = m.groups()
    mantissa = float(f"{int_part}.{frac_part}")
    exp = int(f"{'-' if sign else ''}{exp_digits}")
    if plus_token == "p" and not sign:
        # ``ep<N>`` style — positive exponent. Already covered by sign=""
        # giving a positive integer, so no further work.
        pass
...
def _read_metrics(model_dir: Path) -> pd.DataFrame:
    """Read ``metrics.csv`` and return a DataFrame indexed by epoch with
    per-epoch ``train_loss``, ``val_loss``, ``val_psnr`` columns when
    present.

    The trainer's CSVLogger writes ``<name>_epoch`` and ``<name>_step``
    pairs — we keep the ``_epoch`` rows (one per epoch) and rename them
    back to the bare metric names so downstream rank/plot code can
    work with one canonical schema regardless of Lightning version.
    Step-level NaN rows are dropped during the per-epoch ``max``.
    """
    ...
    # Take the row-wise max across the duplicate (step / epoch) writes
    # in each epoch so the surviving value is the non-NaN scalar logged
    # at epoch end.
    agg = df.groupby("epoch", as_index=False).max(numeric_only=True)
    # Canonicalise ``<name>_epoch`` → ``<name>`` for downstream code.
    rename_map = {
        c: c[: -len("_epoch")]
...
def _final_summary(model_dir: Path) -> dict:
    ...
    if "train_loss" in agg.columns:
        s = agg["train_loss"].dropna()
        if len(s):
            out["train_loss_final"] = float(s.iloc[-1])
    if "val_psnr" in agg.columns:
        s = agg["val_psnr"].dropna()
        if len(s):
            out["val_psnr_best"] = float(s.max())
            out["val_psnr_final"] = float(s.iloc[-1])
...
by_val_psnr = _rank(results, "val_psnr_best", reverse=True)
...
for r in by_val_loss[:top_k]:
    lines.append(
        ...
        f"sched={r['scheduler']!s:<12} epochs={r['epochs_done']}"
    )
if by_val_psnr:
    lines.append("")
    lines.append(f"── BEST BY VAL_PSNR (higher is better, top {top_k}) ──")
    for r in by_val_psnr[:top_k]:
        lines.append(
            f"  val_psnr_best={r['val_psnr_best']:.4f}  "
            f"{r['experiment']:<55} "
            f"opt={r['optimizer']!s:<6} lr={r['learning_rate']!s:<10} "
            f"sched={r['scheduler']!s:<12} epochs={r['epochs_done']}"
        )
...
# Stable colour per learning rate so the same lr looks the same across panels.
lrs_present = sorted(
...
# Combined legend on the rightmost panel.
lr_handles = [
...
fig.suptitle("U-Net sweep — val_loss curves", fontsize=11)
fig.tight_layout()
...
if sorted_by_primary:
    ...
    print("→ pass this folder to predict-unet via `experiment_data_paths.log_path=...`")
    print(
        "→ run sweep_unet_clean.py to trim every sweep dir down to its last checkpoint."
    )
