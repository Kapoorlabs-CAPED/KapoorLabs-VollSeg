"""Trim every ``stardist_sweep_*/`` directory under ``sweep_root`` down
to its final-epoch checkpoint.

Same logic as ``scripts/clean_checkpoints.py`` but applied across the
whole StarDist sweep instead of a single folder. Use after
``sweep_stardist_analyze.py`` when you've decided which sweep
configuration won and just want the last (=best, modulo overfit)
checkpoint of every run on disk.

Sibling of :mod:`sweep_unet_clean.py` — same flow, different
dir-prefix.

Edit the paths block below, then::

    # Dry run first to confirm what would be deleted.
    python sweep_stardist_clean.py
