...
repos. After that, ``kapoorlabs_vollseg.hub.ensure_model`` auto-downloads them.
...
Mapping is read from :data:`kapoorlabs_vollseg.hub.XENOPUS_MODELS`. The local source
...
the matching name from :data:`kapoorlabs_vollseg.hub.XENOPUS_MODELS`.
"""
...
def upload_one(
    ...
    *,
    ...
    if model_name not in XENOPUS_MODELS:
        print(f"  skip: {model_name} not in kapoorlabs_vollseg.hub.XENOPUS_MODELS")
        return False
