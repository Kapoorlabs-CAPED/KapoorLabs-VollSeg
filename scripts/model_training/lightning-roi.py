...
from kapoorlabs_vollseg.care_lightning.trainer import CareInception
from scenario_train_roi import RoiTrainClass
from kapoorlabs_vollseg.care_lightning.pytorch_callbacks import (
    CheckpointModel,
    CustomProgressBar,
)
from kapoorlabs_vollseg.care_lightning.pytorch_loggers import CustomNPZLogger
from kapoorlabs_vollseg.care_lightning.utils import save_config_as_json


configstore = ConfigStore.instance()
configstore.store(name="RoiTrainClass", node=RoiTrainClass)


@hydra.main(config_path="../conf", config_name="scenario_train_roi", version_base="1.3")
def main(config: RoiTrainClass):
    ...
    npz_logger = CustomNPZLogger(save_dir=log_path, experiment_name=experiment_name)
    logger = [npz_logger]
    ...
    # ROI H5 uses raw/mask keys; 2D UNet with conv_dims=2
    trainer.setup_care_h5_datasets(input_key="raw", target_key="mask")
