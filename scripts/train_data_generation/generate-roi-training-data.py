        ...
        for y in range(0, vy - py + 1, sy):
            for x in range(0, vx - px + 1, sx):
                ...
                if len(raw_batch) >= batch_write_size:
                    _write_roi_batch_to_h5(
                        group, "raw", raw_batch, dtype=raw_timelapse.dtype
                    )
                    _write_roi_batch_to_h5(
                        group, "mask", mask_batch, dtype=mask_timelapse.dtype
                    )
                    total += len(raw_batch)
                    raw_batch = []
                    mask_batch = []
...
@hydra.main(
    config_path="../conf", config_name="scenario_generate_roi", version_base="1.3"
)
def main(config: RoiDataClass):
    ...
    h5_output_path = os.path.join(base_data_dir, config.train_data_paths.roi_h5_file)
    ...
    print(
        f"Val fraction: {val_fraction} (last {val_fraction*100:.0f}% of timepoints per timelapse)"
    )
