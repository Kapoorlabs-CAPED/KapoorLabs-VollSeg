...
:class:`vollseg.pipelines.Pipeline` — it doesn't hard-code the legacy
``VollSeg()`` function.
"""
...
class OptimizeThreshold:
    ...
    def run(self, *, tol: float = 1e-2, maxiter: int = 20, verbose: bool = True) -> dict:
        best_prob, best_score, best_nms = None, -np.inf, None
        for nms in self.nms_threshs:
            prob, score = self._optimize_one(nms, tol=tol, maxiter=maxiter, verbose=verbose)
            if score > best_score:
    ...
    def _optimize_one(self, nms_thresh: float, *, tol: float, maxiter: int, verbose: bool):
        np.isscalar(nms_thresh) or _raise(ValueError("nms_thresh must be a scalar"))
        cache: dict = {}

        with tqdm(total=maxiter, disable=not verbose, desc=f"NMS = {nms_thresh:g}") as bar:
            def fn(prob_thresh: float) -> float:
                ...
                bar.update()
                bar.set_postfix_str(f"prob={prob_thresh:.3f} -> {self.measure}={value:.3f}")
                return -value

            opt = minimize_scalar(fn, method="golden", tol=tol, options={"maxiter": maxiter})
