...
def register_model(cls, key, url, hash):
    ...
    if key in models:
        warn(f"re-registering model '{key}' (was already registered for '{cls.__name__}')")
...
def get_registered_models(cls, return_aliases=True, verbose=False):
    ...
    if verbose:
        n = len(models)
        print(f"There {'is' if n == 1 else 'are'} {n} registered "
        ...
        if n > 0:
            ...
            for key in models:
                ...
                aliases_str += "'%s'" % "', '".join(model_aliases[key]) if m > 0 else "None"
