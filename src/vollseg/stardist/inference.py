...
1. **Tile** the input volume via :class:`vollseg._lightning.CarePredictionDataset`.
...
def predict_volume(
    ...
    *,
    ...
    n_tiles
        ...
        :class:`vollseg.CAREDenoiser` (``[1, 4, 4]`` for 3D).
    tile_overlap, batch_size, num_workers, pmin, pmax, device
        As in :class:`vollseg.CAREDenoiser`.
    """
    ...
    prob_map, dist_map = _predict_and_stitch(
        model, image, n_tiles=n_tiles, tile_overlap=tile_overlap,
        batch_size=batch_size, num_workers=num_workers,
        pmin=pmin, pmax=pmax, device=device,
    )

    # 3. Peak detection.
    centers = peak_local_max(
        prob_map, threshold_abs=prob_thresh, min_distance=min_distance,
        exclude_border=False,
    )
    if centers.size == 0:
        return StarDistResult(
            labels=np.zeros(image.shape, dtype=np.uint16),
            prob_map=prob_map, n_objects=0,
        )
    ...
    kept_idx, kept_bbox, kept_masks = _nms_polyhedra(
        centers=centers, dists=dists, scores=scores,
        rays=rays, vol_shape=image.shape, iou_thresh=nms_thresh,
    )
...
def _predict_and_stitch(
    model, image, *, n_tiles, tile_overlap, batch_size, num_workers, pmin, pmax, device,
        ...
        for tiles, coords in loader:
            ...
            for i in range(prob.shape[0]):
                ...
                if image.ndim == 2:
                    ...
                    dist_acc[:, zs:zs + tz, ys:ys + ty, xs:xs + tx] += dists[i] * w[None]
...
def _rasterize_to_bbox(center, rays, dists, vol_shape):
    ...
    if np.any(hi <= lo):
        return tuple(slice(0, 0) for _ in range(ndim)), np.zeros((0,) * ndim, dtype=bool)
    ...
    coords = np.stack([g.ravel() for g in grids], axis=1).astype(np.float32)  # (M, ndim)
