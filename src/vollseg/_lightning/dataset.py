...
:func:`vollseg._lightning.stitch_tiles` matches what the upstream package
...
class CarePredictionDataset(Dataset):
    ...
    def __init__(
        ...
        if volume.ndim != 3:
            raise ValueError(f"CarePredictionDataset expects 3D volume, got ndim={volume.ndim}")
        self.volume = volume
    ...
    def _compute_tile_grid(self):
        tiles = []
        for axis_idx, (vol_size, tile_size) in enumerate(zip(self.volume.shape, self.tile_shape)):
            ...
            tiles = [(s,) for s in starts] if axis_idx == 0 else [t + (s,) for t in tiles for s in starts]
        return tiles
...
def compute_tile_shape(volume_shape: Tuple[int, int, int], n_tiles: Tuple[int, int, int]) -> Tuple[int, int, int]:
