...
class BaseModule(LightningModule):
    ...
    def log_metrics(
        self, name: str, value, on_step: bool = None, on_epoch: bool = None
    ):
        if on_step is None:
...
def _restore_schedulers(scheduler, checkpoint):
    if isinstance(scheduler, WarmCosineAnnealingLR):
        ...
        t_warmup = checkpoint["lr_schedulers"][0]["_schedulers"][0]["total_iters"]
        factor = checkpoint["lr_schedulers"][0]["_schedulers"][0]["start_factor"]
...
def load_checkpoint_file(
    model_path, preffered_checkpoint, map_location, weights_only=False
):
    ...
    if preffered_checkpoint is None:
        if os.path.isdir(model_path):
            ...
            raise ValueError(
                f"Invalid model_path: {model_path}. It must be a valid file or directory."
            )

    checkpoint = torch.load(
        preffered_checkpoint, map_location=map_location, weights_only=weights_only
    )
