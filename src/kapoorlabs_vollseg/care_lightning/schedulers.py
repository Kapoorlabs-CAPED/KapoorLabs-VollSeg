...
class WarmCosineAnnealingLR(_Schedulers):
    ...
    def forward(self, optimizer):
        ...
        return optim.lr_scheduler.SequentialLR(optimizer, schedulers, milestones)
...
class MultiStepLR(_Schedulers):
    def __init__(self, milestones, gamma=0.1, last_epoch=-1):
        ...
        super().__init__(milestones=milestones, gamma=gamma, last_epoch=last_epoch)
...
class ConstantLR(_Schedulers):
    def __init__(self, factor=1.0 / 3, total_iters=5, last_epoch=-1):
        super().__init__(factor=factor, total_iters=total_iters, last_epoch=last_epoch)
