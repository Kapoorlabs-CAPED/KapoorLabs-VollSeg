...
class H5MitosisDataset(Dataset):
    ...
    def _compute_class_weights(self, num_classes):
        ...
        class_weights_dict = {
            class_idx: weight for class_idx, weight in enumerate(class_weights)
        }
...
class H5VisionDataset(Dataset):
    def __init__(
        ...
        if split not in self.h5_handle:
            raise ValueError(
                f"Split '{split}' not found in H5 file. Available: {list(self.h5_handle.keys())}"
            )
        ...
        if "images" not in self.group or "labels" not in self.group:
            raise ValueError(
                f"Split '{split}' must contain 'images' and 'labels' datasets"
            )
    ...
    def _compute_class_weights(self):
        ...
        class_counts = np.bincount(
            class_indices.astype(int), minlength=self.num_classes
        )
    ...
    def get_class_weights(self):
        if self.class_weights_dict is None:
            raise ValueError(
                "Class weights not computed. Set compute_class_weights=True in constructor."
            )
