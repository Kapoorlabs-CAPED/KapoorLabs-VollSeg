"""Tests for vollseg.fusion — watershed_fuse and cellpose_watershed_fuse.
...
from vollseg import cellpose_watershed_fuse, watershed_fuse
...
class TestWatershedFuse:
    def test_3d_smoke(self, labels_3d_two_blobs):
        ...
        out = watershed_fuse(image, stardist_labels=labels_3d_two_blobs,
                             unet_mask=mask, seedpool=True)
    ...
    def test_2d_smoke(self, labels_2d_two_blobs):
        ...
        out = watershed_fuse(image, stardist_labels=labels_2d_two_blobs,
                             unet_mask=mask, seedpool=True)
        assert out.shape == labels_2d_two_blobs.shape

    def test_invalid_ndim_raises(self):
        with pytest.raises(ValueError):
            watershed_fuse(np.zeros((10,)),
...
class TestCellposeWatershedFuse:
    def test_3d_smoke(self, labels_3d_two_blobs):
        ...
        mask = labels_3d_two_blobs > 0
        out = cellpose_watershed_fuse(membrane,
                                      nuclei_labels=labels_3d_two_blobs,
                                      cellpose_mask=mask)
        assert out.shape == labels_3d_two_blobs.shape
